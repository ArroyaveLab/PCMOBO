"""LLM switch policy for the ESTM finite-pool portfolio."""
...
from studies.estm_thermoelectric.constants import ESTM_ARM_NAMES, POLICY_PROMPT_VERSION

ALLOWED_ACQS = ESTM_ARM_NAMES
...
COMMON_PROMPT_PREAMBLE = (
    "You are part of a three-agent portfolio switch for full-observation finite-pool constrained multi-objective "
    "materials optimization. Choose exactly one arm from: qlogpof, qlogehvi, qlognparego, qucb. "
)
COMMON_ARM_SEMANTICS = (
    "Arm semantics: qlogpof=feasibility seeking via explicit constraint satisfaction probability; "
    "qlogehvi=pareto improvement; qlognparego=scalarized tradeoff shaping; qucb=uncertainty exploration. "
)
...
def build_policy_prompts() -> Dict[str, str]:
    ...
    feasibility_suffix = (
        "Your role is to judge which arm best protects feasible discovery right now. Do not assume any fixed "
        "mapping from early or late stages to a specific arm; infer the best arm from the evidence. Return JSON "
        "with keys: recommended_acq, reason, argument, confidence."
    )
    hv_suffix = (
        "Your role is to judge which arm best supports fixed-range feasible hypervolume growth right now. Do not "
        "assume any fixed mapping from early or late stages to a specific arm; infer the best arm from the "
        "evidence. Return JSON with keys: recommended_acq, reason, argument, confidence."
    )
    arbiter_suffix = (
        "Your role is to choose the single arm that best serves the run-level objective over time. Use the "
        "advocate outputs as evidence, not commands. Do not hard-code stage-to-arm rules; resolve the tradeoff "
        "from the raw state and the active policy intent. Return JSON with keys: acq, reason, reflection, confidence."
    )
...
class LLMSwitchPolicy:
        ...
        *,
        ...
        payload = {
            ...
            "best_uncertainty_value": round(
                safe_float(item.get("best_uncertainty_value", 0.0)),
                8,
            ),
            ...
            "chosen_vs_best_hv_gap": round(safe_float(item.get("chosen_vs_best_hv_gap", 0.0)), 8),
        }
        if budget_visible:
    ...
    def _build_payload(self, state: Dict[str, Any]) -> Dict[str, Any]:
        ...
        payload = {
            "metadata": {
                "benchmark": str(metadata.get("benchmark", "estm_thermoelectric")),
            ...
            "global_state": {
                ...
                "cumulative_observed_count": int(
                    global_state.get("cumulative_observed_count", 0) or 0
                ),
    ...
    def _decide_three_agent(self, payload_state: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            feasibility = self._invoke_stage(
                ...
                payload={
                    "task": "Recommend the single arm most justified by feasible discovery right now.",
                    **advocate_payload,
                    "output_format": {
            ...
            arbiter = self._invoke_stage(
                ...
                payload={
                    "task": "Choose the next arm for the ESTM run.",
                    **advocate_payload,
